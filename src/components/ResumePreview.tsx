...
export function ResumePreview({ formData }: ResumePreviewProps) {
  ...
  const downloadAsPDF = async () => {
    try {
      ...
      link.href = url
      link.download = `${formData.personalInfo.name || 'Resume'}.pdf`
    ...
    <div className='space-y-6'>
      <div className='flex justify-between items-center'>
        ...
        <div className='flex gap-2'>
          <Button className='flex items-center gap-1' onClick={printResume} variant='outline'>
          ...
          <Card className='p-8 bg-white'>
            <div className='max-w-[800px] mx-auto space-y-6' ref={resumeRef}>
              ...
              <div className='text-center space-y-2'>
                <h1 className='text-2xl font-bold'>{formData.personalInfo.name || 'Your Name'}</h1>
              ...
              {formData.education.some(edu => edu.institution || edu.degree) && (
                <div className='space-y-3'>
                  <h2 className='text-lg font-semibold border-b border-gray-300 pb-2'>Education</h2>
                  <div className='space-y-4'>
                    {formData.education.map((edu, index) => (
                      <div className='space-y-1' key={edu.id}>
                ...
                <div className='space-y-3'>
                  ...
                  <div className='flex flex-wrap gap-2'>
                    {formatSkills(formData.skills).map((skill, index) => (
                      <span className='bg-gray-100 px-3 py-1 rounded-md text-sm' key={index}>
                        {skill}
                      </span>
                    ))}
                  </div>
          ...
          <Card className='p-6'>
            ...
            <div className='space-y-4'>
              <div>
                ...
                <p>
                  Your resume includes keywords that match the job description. ATS systems scan for
                  these keywords to determine relevance.
                </p>
              </div>
              <div>
                <h4 className='font-medium mb-2'>Formatting</h4>
                <p>
                  This resume uses a clean, simple format that ATS systems can easily parse. Avoid
                  tables, headers/footers, and complex formatting that might confuse ATS.
                </p>
              </div>
              <div>
                <h4 className='font-medium mb-2'>File Format</h4>
                <p>
                  When downloading, save as a PDF or .docx file as these are most compatible with
                  ATS systems.
                </p>
              </div>
              <div>
                ...
                <p>
                  Including metrics and numbers in your achievements helps both ATS and human
                  reviewers understand your impact.
                </p>
              </div>
              <div>
                <h4 className='font-medium mb-2'>Job Title Alignment</h4>
                <p>
                  When possible, match your job titles to those in the job description (if accurate
                  to your experience).
                </p>
              </div>
            </div>
