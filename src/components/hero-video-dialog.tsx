...
function getYouTubeThumbnail(videoSrc: string): string | null {
  // Extract video ID from YouTube embed URL or watch URL
  const embedMatch = videoSrc.match(/youtube\.com\/embed\/([^?&]+)/)
  const watchMatch = videoSrc.match(/youtube\.com\/watch\?v=([^&]+)/)
...
export function HeroVideoDialog({
  ...
  // Auto-derive thumbnail from YouTube URL if not provided
  const resolvedThumbnail = thumbnailSrc || getYouTubeThumbnail(videoSrc) || '/placeholder.svg'
      ...
      <AnimatePresence>
        {isVideoOpen && (
          ...
          >
            ...
            >
              <motion.button type='button' className='absolute -top-16 right-0 text-white text-xl bg-neutral-900/50 ring-1 backdrop-blur-md rounded-full p-2 dark:bg-neutral-100/50 dark:text-black'>
