...
function PlanTodoItem({ todo }: PlanTodoItemProps) {
  ...
  const labelElement = <span className={cn('text-sm', labelClassName)}>{todo.label}</span>

  if (!todo.description) {
    return (
      <li className='-mx-2 flex cursor-default items-start gap-2 rounded-md px-2 py-2'>
        <TodoIcon className={iconClassName} icon={icon} isAnimating={isInProgress} />
        {labelElement}
      </li>
    )
  }
    ...
    <li className='hover:bg-muted -mx-2 cursor-default rounded-md'>
      <Collapsible>
        <CollapsibleTrigger className='group/todo flex w-full cursor-default items-start gap-2 px-2 py-2 text-left'>
          <TodoIcon className={iconClassName} icon={icon} isAnimating={isInProgress} />
          <span className={cn('flex-1 text-sm text-pretty', labelClassName)}>{todo.label}</span>
...
export function Plan({
  ...
  return (
    <>
      ...
      <Card
        className={cn('mb-4 w-full max-w-xl min-w-80 gap-4 py-4', className)}
        data-slot='plan'
        data-tool-ui-id={id}
      >
        <CardHeader className='flex flex-row items-start justify-between gap-4'>
          ...
          {allComplete && <CheckCircle2 className='mt-0.5 size-5 shrink-0 text-emerald-500' />}
        </CardHeader>
          ...
          <div className='bg-muted/70 rounded-lg border px-4 py-3'>
            {showProgress && (
              <>
                ...
                <ProgressBar isCelebrating={allComplete} progress={progress} />
              </>
            )}
