...
export function ExpandableTabs({
  ...
  const handleSelect = (index: number) => {
    if (selected === index) {
      // Second click on already selected tab - execute the action
      onChange?.(index);
    } else {
      // First click - just expand/select the tab
      setSelected(index);
    }
  };
  ...
  return (
    <div
      ref={outsideClickRef}
      className={cn(
        "flex flex-wrap items-center gap-2 border bg-background p-1 shadow-sm",
        className
      )}
    >
      {tabs.map((tab, index) => {
