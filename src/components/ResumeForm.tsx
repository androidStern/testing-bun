    ...
    <div className="flex items-center gap-1">
      {showDictate && onDictate && (
        ...
        >
          <div className={`p-1.5 rounded-full ${isRecording ? 'bg-destructive/10' : 'bg-primary/10'}`}>
            {isRecording ? <Square className="h-4 w-4 text-destructive" /> : <Mic className="h-4 w-4 text-primary" />}
          </div>
      ...
      {showPolish && onPolish && (
        ...
        >
          ...
          <span className="text-[10px] font-medium mt-1 text-primary">{isPolishing ? 'Wait...' : 'Polish'}</span>
        </button>
      )}
    </div>
  );
...
type TranscribeSectionResult = TranscribedSummaryResult | TranscribedWorkResult | TranscribedEducationResult;

// Helper to map extracted resume data to form format
function mapExtractedToFormData(extracted: ResumeExtraction, generateId: () => string): ResumeFormData {
  return {
    education:
    ...
    <div className="flex-1 bg-background p-0 md:p-6 lg:p-8">
      ...
      <div className="max-w-4xl mx-auto bg-card rounded-lg shadow-sm sm:shadow-md p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-muted-foreground">Create an ATS-friendly resume to help you land your next job.</p>
        </div>
